...
#################################################################################
#                                                                               #
#   Program Name : Security Check for LINUX for CentOS                          #
#   Version : V1.03                                                             #
...
#-------------------------------------------------------------------------------#
#                                                                               #
# 본 스크립트의 문제점을 발견시 반드시 lsj8350@gmail.com 으로 연락  바랍니다.#
#                                                                               #
...
echo "U-04) 패스워드 파일 보호 (기존 조치되어있음) cat /etc/shadow"
pwconv
echo "U-44) root이외 UID '0' 금지 (기존 조치되어있음) /etc/passwd"
...
sed -i 's/wheel:x:10:/wheel:x:10:root,securegate/g' /etc/group
...
echo "U-50) 관리자 그룹에 최소한의 계정포함(root, securegate 기본 등록처리 cat /etc/group | grep root"
...
echo "U-05) root홈, 패스 디렉터리 권한 및 패스 설정 (기존 조치되어있음 echo $PATH)"
echo "$PATH"
echo "U-06) 파일 및 디렉터리 소유자 설정 (기존 조치 완료 cat /.bash_profile .bashrc | grep PATH"
...
chown root:root /home/hsck/.bashrc
chmod 644 /etc/profile
chmod 644 /etc/bashrc
chmod 644 /root/.bashrc
chmod 644 /root/.bash_profile
chmod 644 /home/hsck/.bashrc

echo "U-15) world writable 파일 점검 (조치되어있음.점검시 표기되는 부분은 dev부분으로 조치불가, Infiniband dev 사용으로 삭제 불가.아래명령으로확인)"
echo "find / -perm -2 -type d -exec ls -aldL {} \;"

echo "U-16) dev에 존재하지 않는 device 파일 점검(아래명령 수동 확인 필요), 관련부분 조치 불가. 엔진기동에 문제 발생"
echo "find /dev -type f -exec ls -l {} \;"

echo "U-17) $HOME/.rhosts, hosts.equiv 사용 금지 (이미 조치되어있음 파일 없음)"
echo "U-18) 접속 IP 및 포트 제한 (/etc/hosts.allow hosts.deny 파일 수동 조치 필요, 고객사 환경에 따라 조치)"
...
echo "U-58) 홈디렉터리 소유자 및 권한 설정 (조치되어있음)"
echo "U-59) 홈디렉터리로 지정한 디렉토리의 존재 관리 (사용자 계정과 홈 디렉터리의 일치 여부를 점검 조치완료)"
...
echo "U-25) NFS접근 통제(/etc/exports 내용 없음)"
...
echo "U-35) Apache 디렉토리 시스팅 제거(아파치 미사용)"
...
echo "U-39) Aapache 링크 사용 금지 (미사용)"
echo "U-40) Aapache 파일 업로드 및 다운로드 제한 (미사용)"
echo "U-41) Aapache 웹 서비스 영역의 분리 (미사용)"
